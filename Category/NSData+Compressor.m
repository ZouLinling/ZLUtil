...
@implementation NSData (Compressor)
/**
...
-(NSData*) gzipData
{
    ...
    zlibStreamStruct.avail_in  = [self length]; // Number of input bytes left to process
...
//    NSMutableData *compressedData = [NSMutableData dataWithLength:[pUncompressedData length] * 1.01 + 12];
    ...
    NSMutableData *compressedData = [NSMutableData dataWithLength:halflength];
    
    int deflateStatus;
    do
    {
        if (zlibStreamStruct.total_out >= [compressedData length])
            [compressedData increaseLengthBy: halflength];
